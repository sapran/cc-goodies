...
| `voice-notify` | hooks | macOS `say`; Notification + Stop + UserPromptSubmit + PreToolUse(`Agent`) hooks (UserPromptSubmit stamps a `$TMPDIR` turn-start timestamp so Stop can stay quiet on quick turns; PreToolUse on the `Agent` tool speaks a distinct debounced cue when subagents are dispatched, so the subagent-working pause sounds different from a real turn-end) |
