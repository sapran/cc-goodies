...
| `voice-notify` | hooks | macOS `say`; Notification + Stop + UserPromptSubmit + PreToolUse(`Agent`) + SubagentStop hooks (UserPromptSubmit stamps a `$TMPDIR` turn-start timestamp so Stop can stay quiet on quick turns; PreToolUse on the `Agent` tool speaks a distinct debounced cue when subagents are dispatched, so the subagent-working pause sounds different from a real turn-end; PreToolUse + SubagentStop also maintain an in-flight subagent count in two create-only `$TMPDIR` marker dirs so that when `Stop` fires while **background** subagents are still running, it speaks a distinct *waiting* cue instead of a false turn-end sign-off) |
