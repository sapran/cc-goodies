...
| Plugin | What it does |
|--------|--------------|
| **[voice-notify](plugins/voice-notify)** | Speaks a short, natural, first-person cue (macOS `say`) when Claude needs your attention, hands work to subagents, or finishes a long turn — varied phrasing routed by context, a distinct debounced cue while subagents run, quiet on quick turns. |
| **[statusline](plugins/statusline)** | An enriched two-line statusline: `user@host`, cwd, branch/worktree, task focus, model, effort, context % and rate-limit %. Flip it to a single lean line (cwd, branch, model, context %) at runtime with `/statusline-toggle`. Renders the [`caveman`](https://github.com/sapran/caveman) plugin's `[CAVEMAN]` mode badge at the end of the enriched line when active (fail-soft when absent; opt out with `STATUSLINE_CAVEMAN=0`). |
