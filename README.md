...
| Plugin | What it does |
|--------|--------------|
| **[voice-notify](plugins/voice-notify)** | Speaks a short, natural, first-person cue (macOS `say`) when Claude needs your attention, hands work to subagents, or finishes a long turn — varied phrasing routed by context. Subagent cues **name the work**: what was delegated (*"Two helpers: review script changes, and map the hook payloads."*) and what came back (*"Review script changes — done."*), capped on big fan-outs and rolled up when the batch drains. Plus a waiting cue when a turn ends with background work still running — subagents, a workflow, a teammate — with the *"waiting for your input"* cue held back until that work is actually done, and quiet on quick turns. |
