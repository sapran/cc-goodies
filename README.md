...
| Plugin | What it does |
|--------|--------------|
| **[voice-notify](plugins/voice-notify)** | Speaks a short, natural, first-person cue (macOS `say`) when Claude needs your attention, hands work to subagents, runs a long shell command, or finishes a long turn — varied phrasing routed by context. Subagent cues **name the work**: what was delegated (*"Two helpers: review script changes, and map the hook payloads."*) and what came back (*"Review script changes — done."*), capped on big fan-outs and rolled up when the batch drains. Shell commands are announced only when they run long enough to matter (*"Still running: run the test suite."*), including background ones when they finish — and only the description is ever spoken, never the command line. Plus a waiting cue when a turn ends with background work still running, a *stalled* cue when a turn dies on an API error (which otherwise leaves the session silent), a repeating reminder while a permission prompt sits unanswered, and quiet on quick turns. |
...
| **[git-guard](plugins/git-guard)** | Blocks accidental commits/merges/pushes to protected branches (`main`/`master`) before they run. One default behaviour plus an optional block-all-push mode and an opt-in ask-instead-of-block channel for on-branch writes (pushes always block); configurable branches. Cross-platform. |
