...
- [ ] 1.1 Give `deny()` a class parameter (catastrophic | hygiene) alongside the existing
      reason string, so the second output line branches: catastrophic keeps
      `⚠️  This is destructive and IRREVERSIBLE. Verify the target before running.` and
      offers no alternative; hygiene drops that line and prints a `→` safe-alternative
      line specific to the arm.
- [ ] 1.2 Mark every catastrophic call site with the catastrophic class (no text change to
      ...
      `wipefs` / `newfs`, destructive `diskutil`, the `FORK_RE` fork-bomb check,
      `reboot|shutdown|halt|poweroff`, and the `sudo`/`doas`/`su`/`runuser`/`pkexec`/
      `gosu`/`sudoedit`/`setpriv` privilege-escalation arm.
- [ ] 1.3 Mark the hygiene call sites and add each one's safe-alternative text: `chmod
      777`/`0777` → `chmod 755` (or the narrowest mode the task needs); the `TRUNC_RE`
      `: >` idiom → `printf '' >`; `eval` → run the intended command directly instead of
      through `eval`; `detect_net_pipe`'s `curl`/`wget`/`fetch`-into-interpreter match →
...
- [ ] 1.4 Fix the `EXTRA` arm (`shell-guard.sh:276`): interpolate the literal matched
      ...
      its framing neutral — neither the catastrophic irreversibility line nor an invented
      hygiene alternative, since the guard cannot know a user-supplied pattern's severity.
      No change to the `EXTRA` matching mechanism itself.
...
- [ ] 1.6 Confirm the `!`-paste escape-hatch line and the `SHELL_GUARD_DISABLE=1` /
      `/shell-guard` pointer are unchanged and present in every message, catastrophic and
      hygiene alike.
...
- [ ] 3.1 `plugins/shell-guard/tests/run.sh`: stop discarding stderr (the runner currently
      ...
      per case and add content assertions: catastrophic cases assert the `⚠️  ... IRREVERSIBLE` line and
      the absence of a safe-alternative line; hygiene cases assert the specific
      safe-alternative substring for that arm; every blocking case asserts the new
      variants-also-blocked line and the `! $cmd` escape-hatch line are present.
...
- [ ] 3.4 Extend or confirm the existing doc-lint regression test for command-body recipes
      ...
      test comments that the safe-alternative line in shell-guard's own hygiene message
      now names the same fix (`printf '' >`) the doc-lint check enforces.
...
- [ ] 4.1 `plugins/shell-guard/README.md` "What a block looks like": refresh the example
      transcript(s) to show both framings — a catastrophic example (`rm -rf ~`, unchanged
      except the new variants line) and a hygiene example (e.g. `chmod 777 x`) showing the
      dropped irreversibility line, the safe-alternative line, and the variants line.
