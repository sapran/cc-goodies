# statusline-caveman-badge Specification

## Purpose
Render the `caveman` plugin's mode badge (and optional token-savings suffix) at the end of the
statusline's enriched second line, so this statusline and the separate caveman plugin coexist
under Claude Code's single `statusLine` slot. The badge reads the caveman plugin's existing state
files by path, is hardened against terminal-escape injection, opts out via environment variable,
and is fully fail-soft — no badge and no error — when caveman is not installed.
