# voice-notify-agent-identity Specification

## Purpose

Make a subagent fan-out audible as *specific* work rather than an anonymous mass. Every cue
about a subagent names what it is for, taken from the description on its dispatching tool call:
what was handed off, and what came back. A completion is voiced at the point its result reaches
the parent session — exactly once per agent, however the agent was run — with a cap so a wide
fan-out cannot become a monologue and a roll-up so a capped or awaited batch still reports that
it drained. Because the spoken text is model-authored, it is sanitised before it reaches the
speech engine; because several agents can finish at once, cues are serialised. Every fallback
degrades toward a less specific cue, never toward silence or an error.
## Requirements
